...
func (f *crdclient) Create(obj *crd.Policy) (*crd.Policy, error) {
	var result crd.Policy
	err := f.cl.Post().
...
func (f *crdclient) Update(obj *crd.Policy) (*crd.Policy, error) {
	var result crd.Policy
	err := f.cl.Put().
		Namespace(f.ns).Resource(f.plural).
		Body(obj).Do().Into(&result)
...
func (f *crdclient) Get(name string) (*crd.Policy, error) {
	var result crd.Policy
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
...
func (f *crdclient) List(opts metav1.ListOptions) (*crd.PolicyList, error) {
	var result crd.PolicyList
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
